function binary3(c1, c2, maxLoop)
    D   = 2.8 * 1e-6;
    ...
    for n = 3 : maxLoop
        ...
        for iter = 1 : maxIteration
            ...
            for m = 1 : (n - 1)
            p1(n) = 1/2 * sqrt(t) * (sum (c1_s(n - m))/sqrt(m) - 1/2 * c1_s(1)/sqrt(n - 1) - 1/2 * c1_s(n - 2)/sqrt(1)); 
            x_2(n) = 2 * sqrt(D / pi) * (c1 * sqrt(tn) - p1(n));
            
            p2(n) = 1/2 * sqrt(t) * (sum (c2_s(n - m))/sqrt(m) - 1/2 * c2_s(1)/sqrt(n - 1) - 1/2 * c2_s(n - 2)/sqrt(1)); 
            y_2(n) = 2 * sqrt(D / pi) * (c2 * sqrt(tn) - p2(n));
            
            % Step 4: check whether second estimates are within 1% of first estimate
